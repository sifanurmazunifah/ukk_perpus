...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "UPDATE user SET username='$username', password='$password', email='$email', 
              nama_lengkap='$nama_lengkap', alamat='$alamat', role='$role' WHERE id_user=$id";
...
<body>
    <h1>Edit Petugas</h1>
    <form method="POST">
        Username <input type="text" name="username" value="<?= $buku['username']; ?>" required><br>
        Password <input type="text" name="password" value="<?= $buku['password']; ?>"><br>
        Email <input type="text" name="email" value="<?= $buku['email']; ?>"><br>
        Nama Lengkap <input type="text" name="nama_lengkap" value="<?= $buku['nama_lengkap']; ?>"><br>
        Alamat <input type="text" name="alamat" value="<?= $buku['alamat']; ?>"><br>
        Role <input type="text" name="role" value="<?= $buku['role']; ?>"><br>
        <input type="submit" value="Update Petugas">
    </form>
